...
public class BoardDAO {
	...
	public int getListCount(String items, String text) { // items=컬럼명, text=사용자 입력 키워드 문자열
		...
		// 함 //
		if (items == null && text == null) sql = "select count(*) from bookboard";
		else sql = "select count(*) from bookboard where "+items+" like '%"+text+"%'";
	...
	public ArrayList<BoardDTO> getBoardList(int page) {
		...
		String sql = "select * from bookboard order by num desc";
	...
	public ArrayList<BoardDTO> getBoardList(int page, int limit, String items, String text) {
		...
		if(items==null&&text==null) sql = "select * from bookboard order by num desc";
		// 
		else sql = "select * from bookboard where "+items+" like '%"+text+"%' order by num desc";
		...
		try {
			...
			// rs.absolute(index)를 사용하기 위해 매개변수를 sql, ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY 사용
			// => TYPE_SCROLL_INSENSITIVE : rs에서 화면을 이동하는데 사용할 수 있는 커서로 rs.next()를 사용하면 다음 결과 행을 가져오고 다음에는 이전 값을 사용 못하지만 이 옵션을 설정하여 rs를 만들면 한 번 커서가 지나간 다음에도 다시 커서를 앞으로 되돌릴 수 있음
			// => CONCUR_READ_ONLY : rs로 가져온 row의 값을 insert, update로 사용하지 않고 데이터를 읽기만 하겠다
			rs = pstmt.executeQuery();
			
			// rs.absolute(index) : rs 커서를 원하는 위치(index)의 검색 행으로 이동하는 메소드로 rs로 가져온 행의 1,6,11...이런식으로 loop를 돌며 dto객체 생성, 각 컬럼의 데이터를 얻어와 설정
	...
	public void insertBoard(BoardDTO board) {
		...
		try {
			conn = DBConnection.getConnection();
			String sql="insert into bookboard values (num.nextval,?,?,?,?,?,?,?,sysdate,sysdate)";
			pstmt = conn.prepareStatement(sql);
	...
	public BoardDTO getBoardByNum(int num, int page) {
		...
		String sql="select * from bookboard where num=?";
		try {
			conn = DBConnection.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public void updateHit(int num) {
		...
		try {
			conn = DBConnection.getConnection();
			String sql="select hit from bookboard where num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs=pstmt.executeQuery();
			int hit=0;
			...
			sql="update bookboard set hit=? where num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, hit);
			pstmt.setInt(2, num);
			pstmt.executeUpdate();
	...
	public void updateBoard(BoardDTO board) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql="update bookboard set subject=?,content=?,ip=?,update_day=sysdate where num=?";
	...
	public void deleteBoard(int num) {
		...
		String sql="delete from bookboard where num=?";
